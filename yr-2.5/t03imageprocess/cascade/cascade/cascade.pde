...
for (int y=0; y<input.height; y++) {
  for (int x=0; x<input.width; x++) {
    ...
    //cent = color((brightness(a)+brightness(c)+brightness(d)+brightness(f)+brightness(g)+brightness(h)+brightness(i))*0+brightness(e)+(brightness(b)*-1));
    cent = abs(brightness(e)+(brightness(b)*-1));
