...
for (int y=0; y<input.height; y++) {
  for (int x=0; x<input.width; x++) {
    ...
    float sum = color((brightness(cent)-brightness(a)-brightness(b)-brightness(c)-brightness(d)-brightness(f)-brightness(g)-brightness(h)-brightness(i))*(1/9)+(brightness(e)*8/9)*10);
